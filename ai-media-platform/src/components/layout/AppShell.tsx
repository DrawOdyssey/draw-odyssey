...
import {
    ...
    FolderOpen, Film, CreditCard, Coins, ChevronLeft, Menu, Maximize2, Scissors, Palette,
    ChevronDown, Wrench, Brain, Globe, Code2, Users, UserCircle, PenTool, Clapperboard, PersonStanding,
    Bot, Newspaper, User, ShieldCheck,
} from "lucide-react";
import { useState, useEffect } from "react";
import { useAppStore } from "@/lib/store";

const ADMIN_EMAIL = "admin@drawodyssey.com";
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isAdmin = user?.email === ADMIN_EMAIL;
    const accountNav = isAdmin ? [...baseAccountNav, adminNavItem] : baseAccountNav;
  ...
  function NavLink({ href, label, icon: Icon }: { href: string; label: string; icon: any }) {
        ...
        return (
                <Link
                  ...
                  {!collapsed && <span className="text-sm font-medium">{label}</span>span>}
                </Link>Link>
              );
  }
    ...
    function SectionLabel({ label }: { label: string }) {
          ...
          return (
                  <div className="px-3 pt-4 pb-1">
                          <span className="text-[10px] font-semibold text-surface-600 uppercase tracking-widest">{label}</span>span>
                  </div>div>
                );
    }
    ...
    return (
          <div className="min-h-screen flex bg-surface-950">
            ...
            {mobileOpen && (
                ...
                <aside
                          className={`fixed left-0 top-0 h-full bg-surface-900/95 backdrop-blur-xl border-r border-surface-800 z-40 flex flex-col transition-all duration-300
                                    ${collapsed ? "w-16" : "w-60"}
                  ...
                  {/* Logo */}
                        <div className="flex items-center gap-2.5 px-4 h-14 border-b border-surface-800">
                          ...
                          {!collapsed && <span className="text-sm font-bold text-white tracking-tight">Draw Odyssey</span>span>}
                        </div>div>
                        ...
                        <nav className="flex-1 px-2 py-2 space-y-0.5 overflow-y-auto">
                          ...
                          {/* Subscribe */}
                                  ...
                                  <Link
                                    ...
                                    {!collapsed && <span className="text-sm font-medium">Subscribe</span>span>}
                                  </Link>Link>
                        </nav>nav>
                                              ...
                                              <div className="px-2 py-2 border-t border-surface-800 hidden md:block">
                                                        <button
                                                                      ...
                                                                      className="flex items-center gap-3 px-3 py-2 rounded-lg text-surface-500 hover:text-white hover:bg-surface-800 transition-all w-full">
                                                          {collapsed ? <Menu className="w-[18px] h-[18px]" /> : <><ChevronLeft className="w-[18px] h-[18px]" /><span className="text-sm">Collapse</span>span></>>}
                                                        </button>button>
                                              </div>div>
                </aside>aside>
                          ...
                          {/* Mobile hamburger */}
                                  ...
                                  <div className="flex items-center gap-2 md:gap-3">
                                              <div className="flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1.5 bg-surface-800 rounded-full border border-surface-700">
                                                            ...
                                                            <span className="text-sm font-semibold text-white">{user?.credits_balance ?? "--"}</span>span>
                                                            <span className="text-[10px] text-surface-500 hidden sm:inline">credits</span>span>
                                              </div>div>
                                              <Link href="/subscribe" className="btn-primary text-xs !py-1.5 !px-3">
                                                            <span className="hidden sm:inline">Get Credits</span>span>
                                                            <span className="sm:hidden">Credits</span>span>
                                              </Link>Link>
                                  </div>div>
                        </header>header>
                
                        <div className="page-enter">{children}</div>div>
                </main>main>
          </div>div>
        );
}</></Link>
